...
% 存放4个数据集
data_sets = {Accdata, Breakdata, Leftdata, Rightdata};

for i = 1:4  % 对每个数据集进行处理
    ...
    for j = 1:(num_samples / 30)
        ...
        P = cat(3, P, segment_data);  % 拼接到第三维度上，最终 P 尺寸为 30x10x2000
    end
end

% 生成标签（假设：Accdata为1, Breakdata为2, Leftdata为3, Rightdata为4）
labels = [repmat(1, 500, 1); repmat(2, 500, 1); repmat(3, 500, 1); repmat(4, 500, 1)];
% labels 本身为列向量（2000x1）

% 转换为 categorical，保证输出为列向量
t_all = categorical(labels);  % 2000x1 categorical

%% 检查 P 的尺寸，应为 [30, 10, 2000]
disp(size(P));  % 期望 [30, 10, 2000]

%% 划分训练集和测试集
temp = randperm(length(labels));  % 随机打乱索引

% 选择1400个样本为训练集
P_train_numeric = P(:, :, temp(1:1400));
T_train = t_all(temp(1:1400));  % 保持为列向量

% 剩余600个样本为测试集
P_test_numeric = P(:, :, temp(1401:end));
T_test = t_all(temp(1401:end));  % 保持为列向量
...
layers = [ ...
    ...
    reluLayer                           % ReLU激活层
    fullyConnectedLayer(4)              % 全连接层，4个输出类别
